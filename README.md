...
- **Pre-merge verification is controller-side and fails closed.** It runs in
  ...
  claim. The clean review is bound to the PR it was posted on, its HEAD and
  its base branch; `current_pr_url` and the PR GitHub returns for it must
  ...
  report: that PR open at the reviewed HEAD and base, not a draft, no
