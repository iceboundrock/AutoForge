...
| `UPDATE_EPIC` | OpenCode (`update_epic` profile) | `next_issue_url` gets the `INITIALIZING` checks before the controller switches issues: parses as an issue URL of this repo (a foreign URL is never even queried), is neither the EPIC nor the just-finished issue, exists on GitHub and is OPEN. A rejected selection keeps the phase and `resume` asks the agent once more with the reason in its prompt; a second rejection -> `BLOCKED`. Only a verified issue reaches `ANALYZE_EXECUTE`; `null` -> `DONE` |
