...
- **Review SHA binding.** Before every review the controller fetches the PR
  ...
  with the stale round's findings handed to that review as prior findings to
  re-check rather than dropped (no fixer ever resolved them).
...
- **Replanning is controller policy, not reviewer advice.** After a verified
  ...
  and is never reopened, and a conclusive close failure is never adopted. An open source
  found under a recorded intent is decided by GitHub's own timeline: the intent records
  the source PR's `closed` issue-event count, read just before it is persisted, and issue
  events are append-only. A count that rose means a close landed and a human reopened
  the PR (the receipt, if present, says the same), so the reopen stands and nothing is
  closed again; an unchanged count proves no close ever ran, so the resume retries it
  exactly once, through the same pre-close checks, with the attempt persisted first.
...
`ai-implementation` marker (never by branch name or linked issue: shape is
not provenance) and moves to `REVIEW` without running the agent. Two or
more candidate PRs → `BLOCKED` (the controller never guesses); the open
PRs are listed to the end, whatever their number, and a listing the client
cannot read to its end → `BLOCKED` too, since "no PR yet" is then not
...
the interrupted fixer already pushed and goes to `REVIEW` of that HEAD
without running the fixer, because which findings the push resolved is not
...
Git/GitHub state first and not repeat completed operations. The correction
is a re-entry like `resume`: the same GitHub reconciliation (Recovery rules
above) runs before the relaunch, so an agent that created the PR, pushed
the fix, or posted the comment before losing its result block is
reconciled with (the PR adopted, the pushed HEAD reviewed, the comment
handed over) rather than relaunched. In LOCAL mode a
...
refuses further launches until the log directory is repaired. In every one
of these the agent may already have posted a comment, pushed or opened a PR
that the controller state does not record; `resume` re-enters the phase and
reconciles with GitHub first (Recovery rules above), and the resumed agent
is told to inspect the real Git/GitHub state in its prompt.
...
- **Pre-merge verification is controller-side and fails closed.** It runs in
  ...
  claim. The clean review is bound to the PR it was posted on, its HEAD, its
  base branch and its merge base; `current_pr_url` and the PR GitHub returns for it must
  be that PR by identity (repository and number), else `BLOCKED` before
  anything else is read (the same commits proposed as another PR against
  another base are a change no review decided on). GitHub must then
  report: that PR open at the reviewed HEAD and base from the reviewed
  merge base (a base rewritten under its name goes back to `REVIEW`;
  ordinary commits landing on the base do not move it), not a draft, no
...
- **Post-merge is reconciled from GitHub.** The merge is counted only after
  ...
  verification and the write, so `--match-head-commit` refused it) or against
  a different base (retargeted in that window) and no
