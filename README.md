...
| `FIX` | Claude Code (`fable`, effort high) | before the fixer runs, the PR HEAD must still be the reviewed HEAD the findings are bound to; a HEAD already past it (an unrecorded fix, an operator push) goes back to `REVIEW` without launching the fixer. Afterwards: `previous_head_sha` == current HEAD, every open finding ID resolved (`fixed` / `follow_up_created` / `no_change_with_rationale`), follow-up issues exist in this repo and are OPEN, actual PR HEAD == `new_head_sha`, a `fixed` resolution moved HEAD |
...
or more candidate PRs → `BLOCKED` (the controller never guesses). `REVIEW`
and `FIX` re-enter the same way: `REVIEW` finds a comment the interrupted
reviewer already posted for this round at this HEAD and hands it to the
reviewer to adopt (two such comments → `BLOCKED`); `FIX` finds a HEAD the
interrupted fixer already pushed and goes to `REVIEW` of that HEAD without
running the fixer, because which findings the push resolved is not inferred.
